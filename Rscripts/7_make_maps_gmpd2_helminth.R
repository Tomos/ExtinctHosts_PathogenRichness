...
hp3_orders = c('CARNIVORA', 'CETARTIODACTYLA', 'PRIMATES')
# Read HP3 results
# Get HP3 all virus predictions
tag <- "noExtinction"
repo_path <- "GMPD2_helminths"
repo <- "GMPD2"
reso <- 1/6
reso_txt <- "Reso_sixth"
all_helminth_gam <- readRDS(paste0(base_dir, "intermediates/noExtinction_gmpd2_helminth_models_nb.rds"))$model[[1]]
hp3_hosts <- read_rds(paste0(base_dir, "intermediates/GMPD2_helminth_dataset.rds"))
hp3_all = as_tibble(left_join(all_helminth_gam$model, hp3_hosts))
hp3_all_pred = within(hp3_all, {hDiseaseZACitesLn = max(hDiseaseZACitesLn)})
hp3_all = mutate(hp3_all,
                 observed = TotVirusPerHost,
                 predicted = as.vector(unname(predict(all_helminth_gam, hp3_all, type="response"))),
                 predicted_max = as.vector(unname(predict(all_helminth_gam, hp3_all_pred, type="response"))),
...
data_hp3_all = data_hp3_all[!is.na(data_hp3_all$Order),]
...
helminth_missing <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-map_intermediate_", repo, "-", "helminth-missing"))
helminth_number <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-map_intermediate_", repo, "-", "helminth-number"))
helminth_observed <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-map_intermediate_", repo, "-", "helminth-observed"))
helminth_predicted <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-map_intermediate_", repo, "-", "helminth-predicted"))
helminth_predicted_max <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-map_intermediate_", repo, "-", "helminth-predicted_max"))
helminth_resid <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-map_intermediate_", repo, "-", "helminth-resid"))
...
carn_helminth_bias <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-", "map_intermediate_", repo, "-helminth-CARNIVORA-BIAS"))
cert_helminth_bias <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-", "map_intermediate_", repo, "-helminth-CETARTIODACTYLA-BIAS"))
prim_helminth_bias <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-", "map_intermediate_", repo, "-helminth-PRIMATES-BIAS"))
all_helminth_bias <- readRDS(paste0(base_dir, "maps/", tag, "-", reso_txt, "-", "map_intermediate_", repo, "-helminth-ALL-BIAS"))
...
saveRDS(rasters4, paste0(base_dir, "intermediates/", tag, "-GMPD2_helminth_rasters_ALL.rds"))
